...
innereEingabeZeile([FeldNo|Rest]) -->
	html([tr([td(input([name('systemName' + FeldNo), type("text"), maxlength("40"), pattern("^(?!System$)\\S*$")])),
...
fehlerSystemNameReserviert :-
   	server:holeCssAlsStyle(StyleString),
	TermerizedHead = [\[StyleString], title('No mans Sky trainer: Planeteneigenschaften Fehler')],
	TermerizedBody = [
		\['<redHeader>'],
		h3(align(center), 'Der Systemname "System" ist reserviert. Bitte wählen Sie einen anderen'),
		\['</redHeader>']
		             ],
	reply_html_page(TermerizedHead, TermerizedBody).
